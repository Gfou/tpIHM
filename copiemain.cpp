...
int map(int x, int oldx1, int oldx2, int newx1, int newx2){
	int diffnew=newx2-newx1;
	int diffold=oldx2-oldx1;
	int rapport=newx2/oldx2;
	x*=rapport;
	return x+=newx1;
}
...
int main(int, char**)
{
	...
	vector< Rect_<int> > faces;    				//on crée un tableau de rectangle
		...
		printf("gauche: %d droite: %d haut: %d bas: %d\n",map(gauche,5,30,5,width),map(droite,5,30,5,width),map(haut,5,30,5,height),map(bas,5,30,5,height));
		
		n=width/2;
		l=height/2;
        	n=n+(map(droite,5,30,5,width/2)-map(gauche,5,30,5,width/2));
		l=l-(map(haut,5,30,5,height/2)-map(bas,5,30,5,height/2));
